...
%Jonas
The overall design of the program, is setup from the use of MVVMLight.

\subsubsection{Design patterns}

The program uses the MVVM design pattern. In this case, there is only one
viewmodel, namely the MainViewModel. This takes care of all the actions/commands
...
Furthermore is the singleton pattern used for the undoredo controller, which
utilizes the command pattern.

The observer pattern is used with MVVMLight to notify GUI components when they
rely on each other, as well as for button clicks.
